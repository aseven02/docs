...
There are six main patterns for building multi-agent systems, each suited to different use cases:
...
These patterns share common coordination mechanisms. **Tool calling** is central—tools invoke sub-agents ([supervisor](#supervisor), [spawning subagents](#spawning-subagents)), update state to trigger routing ([state machine](#state-machine)), load context on-demand ([skills](#skills)), or invoke multi-agent systems (wrapping a [router](#router) as a tool).
...
The key mechanism: tools use @[`Command`] to update `current_step`, then middleware reads this state and dynamically configures the agent's system prompt and available tools on each turn.
