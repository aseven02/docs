---
title: Build a customer support state machine
...
<Tip>
Some users may have implemented this workflow in the past using handoffs between distinct agents (an approach popularized by OpenAI). However, this can introduce unnecessary complexity (and latency) and ambiguity in correctly handling the chat history: an extra tool call to transfer control, followed by another to act on the user's request. This state machine pattern is more efficient—one tool call both performs the action (like recording warranty status) and transitions to the next step. Users experience a single, continuous agent throughout.
</Tip>
